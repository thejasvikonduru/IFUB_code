...
class Biconn_algo:public Diameter_static
{
    public:
        ...
        ll biconn_new_algo(Biconn_ops g)
        {
            ...
            cout<<"No of articulation points:"<<g.arts.size()<<endl;
            clock_t end2=clock();
            ...
            diam=ifub_new(source,four_sweep_info.second,bcc_graphs,bcc_graphs_static,&g);
            clock_t end5=clock();
        ...
        ll ifub_helper_new(vector<ll> &vertices_subset,vector<Biconn_algo*> &bcc_graphs,vector<Diameter_static*> bcc_graphs_static,Biconn_ops* g)
        {
            ll max_ecc=INT_MIN;
            ll curr_vert;
            ll bcc_id;
            ...
            {
                cout<<vertices_subset[i]<<" "<<(*g).arts[vertices_subset[i]]<<endl;
                ...
                if(vertex_bcc.find(curr_vert)!=vertex_bcc.end())
                {
                    ...
                    if(((*g).biconn_artics[bcc_id]).size()==1&&curr_bcc_visited[bcc_id]==0)
                    {
                        ...
                        max_ecc=max(max_ecc,(*gr).max_level+(*g).bct_traversal_art_point(bcc_graphs_static,(*g).biconn_artics[bcc_id][0],block_visited));
                ...
                else
                {
                    ...
                    ll curr_art_diam=(*g).bct_traversal_art_point(bcc_graphs_static,curr_vert,block_visited);
                    max_ecc=max(max_ecc,curr_art_diam);
                }
            }
            return max_ecc;
        }
        ll ifub_new(ll source,ll lb,vector<Biconn_algo*> &bcc_graphs,vector<Diameter_static*> bcc_graphs_static,Biconn_ops* g)
        {
            bcc_visited.clear();
            BFS(source,0);
            ll ub=2*max_level;
            ...
            while(ub-lb > k)
            {
                curr_max_ecc=ifub_helper_new(level_vertices_source[curr_level],bcc_graphs,bcc_graphs_static,g);
